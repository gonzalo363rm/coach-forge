...
import { updateClubMemberAction } from "@/app/actions/club"
...
export function ClubMemberEditForm({ member, clubName }: Props) {
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<ClubMemberUpdateInput>({
        resolver: zodResolver(clubMemberUpdateSchema),
        defaultValues: {
            id: member.id,
    ...
    function onSubmit(values: ClubMemberUpdateInput) {
        setServerError(null)
        startTransition(async () => {
            const result = await updateClubMemberAction(values)
            if (!result.ok) {
