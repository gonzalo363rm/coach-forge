...
function elementImageFilename(elementId: string, ext: string): string {
    return `element-${elementId}.${ext}`
}

export function elementImagePublicUrl(elementId: string, ext: string): string {
    return `/elements/${elementImageFilename(elementId, ext)}`
}

function elementImageDiskPath(elementId: string, ext: string): string {
    return path.join(process.cwd(), "public", "elements", elementImageFilename(elementId, ext))
}

function extFromImagePath(imagePath: string): string | null {
...
async function saveElementImageFile(
    ...
    const filePath = elementImageDiskPath(elementId, ext)
    await mkdir(path.dirname(filePath), { recursive: true })
    await writeFile(filePath, bytes)

    return elementImagePublicUrl(elementId, ext)
}

async function removeElementImageFiles(elementId: string): Promise<void> {
    await Promise.all(
        Object.values(MIME_EXT).map((ext) =>
            unlink(elementImageDiskPath(elementId, ext)).catch(() => {
                /* opcional */
            }),
        ),
    )
    ...
    try {
        ...
        if (imageBase64 !== undefined && imageMime !== undefined) {
            try {
                ...
                const oldExt = extFromImagePath(current.image)
                const newExt = MIME_EXT[imageMime]
                if (oldExt && newExt && oldExt !== newExt) {
                    await unlink(elementImageDiskPath(id, oldExt)).catch(() => {})
                }
...
export async function elementDelete(id: string): Promise<ElementMutationResult> {
    try {
        ...
        if (deleted.image.startsWith("/elements/")) {
